...
export async function GET(
  ...
  try {
    ...
    log({ level: "info", module: "backend-store-customer-lookup", operation: "lookupCustomer", phase: "start", filterKeys: Object.keys(filter).join(",") })
    ...
    log({ level: "info", module: "backend-store-customer-lookup", operation: "queryExtensions", phase: "step", duration: Date.now() - t1, filterKeys: Object.keys(filter).join(","), extensionCount: extensions.length })

    if (extensions.length === 0) {
      log({ level: "info", module: "backend-store-customer-lookup", operation: "lookupCustomer", phase: "response", duration: Date.now() - t0, filterKeys: Object.keys(filter).join(","), status: "not_found" })
    ...
    try {
      ...
      log({ level: "info", module: "backend-store-customer-lookup", operation: "retrieveCustomer", phase: "step", duration: Date.now() - t2, customerId: ext.customerId })
    } catch (err) {
      logger?.error?.(
        `Customer lookup failed for extension ${ext.id}: ${(err as Error).message}`
      )
      log({ level: "error", module: "backend-store-customer-lookup", operation: "retrieveCustomer", phase: "step", duration: Date.now() - t0, extensionId: ext.id, error: (err as Error).message })
      return res.json({ customer: null })
    }

    log({ level: "info", module: "backend-store-customer-lookup", operation: "lookupCustomer", phase: "response", duration: Date.now() - t0, filterKeys: Object.keys(filter).join(","), customerId: customer.id })
    ...
    console.error(JSON.stringify({
      ...
      module: "backend-store-customer-lookup",
